...
function Wo_RegisterPoint($post_id, $type, $action = '+', $user_id = 0) {
    ...
    if ($wo['config']['point_allow_withdrawal'] == 1) {
        $query_one = "UPDATE " . T_USERS . " SET `points` = '{$points_amount}',`daily_points` = '{$daily_points}', `balance` = '{$balance_amount}', `converted_points` = '{$converted_points}' WHERE `user_id` = {$user_id} ";
    } else {
        $query_one = "UPDATE " . T_USERS . " SET `points` = '{$points_amount}',`daily_points` = '{$daily_points}', `wallet` = '{$wallet_amount}', `converted_points` = '{$converted_points}' WHERE `user_id` = {$user_id} ";
    }
...
function Wo_UpdateBalance($user_id = 0, $balance = 0, $type = '+',$withdrawal = '') {
    global $wo, $sqlConnect;
    if (empty($user_id) || !is_numeric($user_id) || $user_id < 0) {
        return false;
    }
    ...
    if ($type == '+') {
        ...
        if (!empty($withdrawal)) {
            if ($balance == 0 && $wo['config']['point_allow_withdrawal'] == 1) {
                ...
                $c_points = $c_balance * $wo['config']['dollar_to_point_cost'];
    ...
    $query_one = "UPDATE " . T_USERS . " SET `balance` = '{$balance}' ,  `points` = '{$points}' ,  `converted_points` = '{$converted_points}' WHERE `user_id` = {$user_id} ";
    $query     = mysqli_query($sqlConnect, $query_one);
    if ($query) {
        cache($user_id, 'users', 'delete');
        return true;
...
function Wo_GetForumSec($args = array()) {
    ...
    if (mysqli_num_rows($sql_query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($sql_query_one)) {
            $fetched_data['section_name_lang'] = Wo_ReplaceText($fetched_data['section_name']);
            $fetched_data['description_lang'] = Wo_ReplaceText($fetched_data['description']);
            if ($forums) {
...
function Wo_GetForums($args = array()) {
    ...
    while ($fetched_data = mysqli_fetch_assoc($sql_queryset)) {
        $fetched_data['name_lang'] = Wo_ReplaceText($fetched_data['name']);
        $fetched_data['description_lang'] = Wo_ReplaceText($fetched_data['description']);
            ...
            if (empty($get_session_data) || !empty($get_session_data)) {
                ...
                if (!empty($to_data['android_n_device_id']) || !empty($to_data['ios_n_device_id']) || !empty($to_data['web_device_id'])) {
                    ...
                    if (!empty($sql_get_notification_for_push['type'])) {
                        ...
                        if ($sql_get_notification_for_push['type'] == 'friends_request') {
                            $sql_get_notification_for_push['type_text'] .= $wo['lang']['friends_request'];
                        }
                        if ($sql_get_notification_for_push['type'] == 'chat_group_mention') {
                            $sql_get_notification_for_push['type_text'] .= $wo['lang']['chat_group_mention'];
                        }
...
function Wo_GetMovies($args = array()) {
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            ...
            $fetched_data['url']    = Wo_SeoLink("index.php?link1=watch-film&film-id=" . $fetched_data['id']) . '_' . Wo_SlugPost($fetched_data['name']);
            $data[]                 = $fetched_data;
        }
    }
    return $data;
...
function Wo_DeleteUserAd($id = false) {
    ...
    if($ad->appears == 'story' || $ad->appears == 'entire') {
        $story = $db->where('ad_id', $id)->getOne(T_USER_STORY);
        if (!empty($ad) && !empty($ad->ad_media)) {
            Wo_DeleteStatus($story->id);
        }
    }
...
function Wo_GetMytransactions($args = array()) {
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            ...
            if ($fetched_data['kind'] == 'PRO') {
                $note = json_decode($fetched_data['notes'],true);
                if (!empty($note)) {
                    $fetched_data['notes'] = $note;
                    if (in_array($fetched_data['notes']['pro_type'], array_keys($wo['pro_packages']))) {

                        $pro_name = $wo["pro_packages"][$fetched_data['notes']['pro_type']]['name'];
                        $fetched_data['notes'] = str_replace('{text}', $pro_name . $wo['lang'][$fetched_data['notes']['method_type']], $wo['lang']['trans_upgrade_to_pro']);

                    }
                    else{
                        $fetched_data['notes'] = str_replace('{text}', $wo['lang'][$fetched_data['notes']['method_type']], $wo['lang']['trans_upgrade_to_pro']);
                    }
                }
                else{
                    $fetched_data['notes'] = $wo['lang']['upgrade_to_pro'];
                }
            ...
            if ($fetched_data['kind'] == 'SALE') {
                $fetched_data['notes'] = $wo['lang']['product_sale'];
            }
            $data[] = $fetched_data;
        }
    }
...
function Wo_GetAlddAdIdsByType($type = 'post', $last_id = 0) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function Wo_CreateGChat($name = false, $parts = array(),$type = 'group') {
    ...
    $time  = time();
    $id    = false;
    $sql   = "INSERT INTO " . T_GROUP_CHAT . " (`group_id`,`user_id`,`group_name`,`type`,`time`) VALUES (null,'$user','$name','$type','$time')";
    $query = mysqli_query($sqlConnect, $sql);
    if ($query) {
        $id = mysqli_insert_id($sqlConnect);
        if ($id && is_numeric($id)) {
...
function Wo_CheckLastGroupUnread() {
    ...
    $groups = array();
    $time   = time();
    $sql    = "SELECT `last_seen`,`group_id` FROM " . T_GROUP_CHAT_USERS . " WHERE `user_id` = {$user} AND `active` = '1' AND `group_id` IN (SELECT `group_id` FROM " . T_GROUP_CHAT . " WHERE `type` = 'group') ";
    $query  = mysqli_query($sqlConnect, $sql);
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $last_message = Wo_GetChatGroupLastMessage($fetched_data['group_id']);
...
function Wo_UserRelationship($user = false) {
    ...
    if (mysqli_num_rows($query) > 0) {
        ...
        if ($fetched_data['relationship'] == 2) {
            $relation = $wo['lang']['relation_with'] . ' <a href="'.$with_data['url'].'" data-ajax="?link1=timeline&u='.$with_data['username'].'">@' . $with_data['name'].'</a>';
        } else if ($fetched_data['relationship'] == 3) {
            $relation = $wo['lang']['married_to'] . ' <a href="'.$with_data['url'].'" data-ajax="?link1=timeline&u='.$with_data['username'].'">@' . $with_data['name'].'</a>';
        } else if ($fetched_data['relationship'] == 4) {
            $relation = $wo['lang']['engaged_to'] . ' <a href="'.$with_data['url'].'" data-ajax="?link1=timeline&u='.$with_data['username'].'">@' . $with_data['name'].'</a>';
        }
...
//        $data_array['limit'] . "GROUP BY user_id, title, description, posted, expire, thumbnail, ad_id";
    ...
    while ($fetched_data = mysqli_fetch_assoc($query_run)) {
        ...
        $not_seen = $db->where("(user_id = ".$fetched_data['user_id']." AND id NOT IN (SELECT story_id FROM ".T_STORY_SEEN." WHERE user_id = ".$wo['user']['id']."))")->getValue(T_USER_STORY,'COUNT(*)');
...
function FFMPEGUpload($data) {
    ...
    $video_output_full_path_water  = $dir . "/" . $video_path_water;
    ...
    if (empty($data['video_thumb'])) {
        ...
        if ($wo['config']['amazone_s3'] == 1 || $wo['config']['wasabi_storage'] == 1 || $wo['config']['ftp_upload'] == 1 || $wo['config']['spaces'] == 1 || $wo['config']['cloud_upload'] == 1 || $wo['config']['backblaze_storage'] == 1) {
            if (!empty($_POST) && !empty($_POST['postPrivacy']) && $_POST['postPrivacy'] == 6) {
                ...
                $db->where('post_id',$data['id'])->update(T_POSTS,['blur_url' => $blur_url]);
                Wo_UploadToS3($blur_url);
            }
    ...
    if ($data['post_data']['postPrivacy'] == 6) {
        if($file_thumb) {
            $update_data['blur_url'] = blur_image($file_thumb);
        }
    }

    $db->where('id', $data['id'])->update(T_POSTS, $update_data);
...
function TextForMode($value) {
    ...
    $wo['website_modes_text'] = array(
        ...
        'twitter' => array(
            ...
            'generate_ai_post' => 'generate_ai_tweet',
            'saved_posts' => 'saved_tweets',
            'create_post' => 'tweet',
            'share_new_post' => 'share_new_tweet',
            'users_shared_post' => 'users_shared_tweet',
            'shared_a_post' => 'shared_a_tweet',
        ),
        'instagram' => array(
            'search_header_label' => 'search_header_people'
        )
...
function GetModeBtn($value) {
    ...
    $wo['website_modes_btn'] = array(
        ...
        "instagram" => array(
            "liked_btn" => "<span class='active-like'><svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' viewBox='0 0 24 24'><path d='M1.25 9.00042C1.25 5.04651 4.62678 2.25064 8 2.25064C9.37536 2.25064 10.86 2.64677 11.9984 3.81005C13.1283 2.6447 14.5981 2.23028 16.0109 2.25072C19.3659 2.29926 22.75 5.03776 22.75 9.00042C22.75 15.0194 16.9805 19.3484 12 21.8385C7.01949 19.3484 1.25 15.0194 1.25 9.00042Z' fill='currentColor'/></svg></span>",
            "like_btn" => "<svg width='24' height='24' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none'><path d='M12 21C13 21 22 16.0003 22 9.00046C22 5.5006 19 3.04408 16 3.00068C14.5 2.97897 13 3.50068 12 5.00062C11 3.50068 9.47405 3.00068 8 3.00068C5 3.00068 2 5.5006 2 9.00046C2 16.0003 11 21 12 21Z' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'/></svg>",
            "comments_btn" => "<svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' viewBox='0 0 24 24'><path fill='currentColor' d='M2.31934 16.6807C2.29299 16.6244 2.26675 16.5684 2.24087 16.5125C1.60482 15.1395 1.25 13.6102 1.25 12C1.25 6.06294 6.06294 1.25 12 1.25C17.9371 1.25 22.75 6.06294 22.75 12C22.75 17.9371 17.9371 22.75 12 22.75C10.786 22.75 9.72211 22.5765 8.72366 22.2194C7.83036 21.8998 7.46428 21.7705 7.35382 21.7445C6.68259 21.5866 6.0471 21.8925 5.41652 22.196C5.00937 22.392 4.60427 22.587 4.1929 22.6555C3.06842 22.8429 2.06787 21.9222 2.16118 20.7861C2.19452 20.3801 2.35166 20.0332 2.50977 19.6842C2.61162 19.4593 2.71389 19.2336 2.78372 18.9906C3.01855 18.1734 2.66113 17.4103 2.31934 16.6807ZM8 9.25C7.58579 9.25 7.25 9.58579 7.25 10C7.25 10.4142 7.58579 10.75 8 10.75H16C16.4142 10.75 16.75 10.4142 16.75 10C16.75 9.58579 16.4142 9.25 16 9.25H8ZM8 13.25C7.58579 13.25 7.25 13.5858 7.25 14C7.25 14.4142 7.58579 14.75 8 14.75H12C12.4142 14.75 12.75 14.4142 12.75 14C12.75 13.5858 12.4142 13.25 12 13.25H8Z' /></svg>",
            "share_btn" => "<svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' viewBox='0 0 24 24'><path fill='currentColor' d='M20.3591 12.7315C19.9891 12.7315 19.6791 12.4515 19.6391 12.0815C19.3991 9.88154 18.2191 7.90154 16.3991 6.64154C16.0691 6.41154 15.9891 5.96154 16.2191 5.63154C16.4491 5.30154 16.8991 5.22154 17.2291 5.45154C19.3991 6.96154 20.7991 9.32154 21.0891 11.9315C21.1291 12.3315 20.8391 12.6915 20.4391 12.7315C20.4091 12.7315 20.3891 12.7315 20.3591 12.7315Z' /><path fill='currentColor' d='M3.73931 12.7812C3.71931 12.7812 3.68931 12.7812 3.66931 12.7812C3.26931 12.7412 2.97931 12.3812 3.01931 11.9812C3.28931 9.37118 4.66931 7.01118 6.81931 5.49118C7.13931 5.26118 7.59931 5.34118 7.82931 5.66118C8.05931 5.99118 7.97931 6.44118 7.65931 6.67118C5.85931 7.95118 4.68931 9.93118 4.46931 12.1212C4.42931 12.5012 4.10931 12.7812 3.73931 12.7812Z' /><path fill='currentColor' d='M15.9906 21.1003C14.7606 21.6903 13.4406 21.9903 12.0606 21.9903C10.6206 21.9903 9.25059 21.6703 7.97059 21.0203C7.61059 20.8503 7.47059 20.4103 7.65059 20.0503C7.82059 19.6903 8.26059 19.5503 8.62059 19.7203C9.25059 20.0403 9.92059 20.2603 10.6006 20.3903C11.5206 20.5703 12.4606 20.5803 13.3806 20.4203C14.0606 20.3003 14.7306 20.0903 15.3506 19.7903C15.7206 19.6203 16.1606 19.7603 16.3206 20.1303C16.5006 20.4903 16.3606 20.9303 15.9906 21.1003Z' /><path fill='currentColor' d='M12.0505 2.01172C10.5005 2.01172 9.23047 3.27172 9.23047 4.83172C9.23047 6.39172 10.4905 7.65172 12.0505 7.65172C13.6105 7.65172 14.8705 6.39172 14.8705 4.83172C14.8705 3.27172 13.6105 2.01172 12.0505 2.01172Z' /><path fill='currentColor' d='M5.05047 13.8711C3.50047 13.8711 2.23047 15.1311 2.23047 16.6911C2.23047 18.2511 3.49047 19.5111 5.05047 19.5111C6.61047 19.5111 7.87047 18.2511 7.87047 16.6911C7.87047 15.1311 6.60047 13.8711 5.05047 13.8711Z' /><path fill='currentColor' d='M18.9509 13.8711C17.4009 13.8711 16.1309 15.1311 16.1309 16.6911C16.1309 18.2511 17.3909 19.5111 18.9509 19.5111C20.5109 19.5111 21.7709 18.2511 21.7709 16.6911C21.7709 15.1311 20.5109 13.8711 18.9509 13.8711Z' /></svg>",
            "like_icon" => "<svg width='24' height='24' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none'><path d='M12 21C13 21 22 16.0003 22 9.00046C22 5.5006 19 3.04408 16 3.00068C14.5 2.97897 13 3.50068 12 5.00062C11 3.50068 9.47405 3.00068 8 3.00068C5 3.00068 2 5.5006 2 9.00046C2 16.0003 11 21 12 21Z' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'/></svg>",
            "liked_comment" => "<svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' viewBox='0 0 24 24' class='feather'><path d='M1.25 9.00042C1.25 5.04651 4.62678 2.25064 8 2.25064C9.37536 2.25064 10.86 2.64677 11.9984 3.81005C13.1283 2.6447 14.5981 2.23028 16.0109 2.25072C19.3659 2.29926 22.75 5.03776 22.75 9.00042C22.75 15.0194 16.9805 19.3484 12 21.8385C7.01949 19.3484 1.25 15.0194 1.25 9.00042Z' fill='currentColor'/></svg>",
            "like_comment" => "<svg width='24' height='24' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' class='feather' fill='none'><path d='M12 21C13 21 22 16.0003 22 9.00046C22 5.5006 19 3.04408 16 3.00068C14.5 2.97897 13 3.50068 12 5.00062C11 3.50068 9.47405 3.00068 8 3.00068C5 3.00068 2 5.5006 2 9.00046C2 16.0003 11 21 12 21Z' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'/></svg>",
        ),
    );
    ...
    if ($wo['config']['theme'] == 'sunshine') {
        $btns = array(
            ...
            'comments_btn' => '<svg height="682pt" viewBox="-21 -47 682.66669 682" width="682pt" xmlns="http://www.w3.org/2000/svg" class="feather feather-message-circle" fill="currentColor"><path d="m552.011719-1.332031h-464.023438c-48.515625 0-87.988281 39.464843-87.988281 87.988281v283.972656c0 48.414063 39.300781 87.816406 87.675781 87.988282v128.863281l185.191407-128.863281h279.144531c48.515625 0 87.988281-39.472657 87.988281-87.988282v-283.972656c0-48.523438-39.472656-87.988281-87.988281-87.988281zm50.488281 371.960937c0 27.835938-22.648438 50.488282-50.488281 50.488282h-290.910157l-135.925781 94.585937v-94.585937h-37.1875c-27.839843 0-50.488281-22.652344-50.488281-50.488282v-283.972656c0-27.84375 22.648438-50.488281 50.488281-50.488281h464.023438c27.839843 0 50.488281 22.644531 50.488281 50.488281zm0 0"/><path d="m171.292969 131.171875h297.414062v37.5h-297.414062zm0 0"/><path d="m171.292969 211.171875h297.414062v37.5h-297.414062zm0 0"/><path d="m171.292969 291.171875h297.414062v37.5h-297.414062zm0 0"/></svg> <span class="like-btn-mobile">' . $wo['lang']['comment_post_label'] . '</span>',
            'share_btn' => '<svg class="feather feather-share" enable-background="new 0 0 512.231 512.231" height="512" viewBox="0 0 512.231 512.231" width="512" xmlns="http://www.w3.org/2000/svg" fill="currentColor"><g><path d="m271.249 121.448c-70.515 3.65-136.443 33.004-187.025 83.587-54.152 54.151-83.975 125.899-83.975 202.025v105.171l38.098-87.825c44.499-88.669 134.739-147.458 232.902-152.927v121.202l240.733-196.638-240.733-196.043zm30 29.612v-87.94l163.267 132.958-163.267 133.361v-88.379h-15c-56.878 0-112.732 15.993-161.522 46.249-36.948 22.913-68.428 53.121-92.593 88.604 15.408-126.529 123.493-224.853 254.115-224.853z"/></g></svg> <span class="like-btn-mobile">' . $wo['lang']['share'] . '</span>',
            'like_icon' => '<svg xmlns="http://www.w3.org/2000/svg" width="58.553" height="58.266" viewBox="0 0 58.553 58.266" class="feather"> <path d="M-7080.317,1279.764l-26.729-1.173a1.657,1.657,0,0,1-1.55-1.717l1.11-33.374a4.112,4.112,0,0,1,2.361-3.6l.014-.005a13.62,13.62,0,0,1,1.978-.363h.007a9.007,9.007,0,0,0,3.249-.771c2.645-1.845,3.973-4.658,5.259-7.378l.005-.013.031-.061.059-.13.012-.023c.272-.576.61-1.289.944-1.929l0-.007c.576-1.105,2.327-4.46,4.406-5.107a2.3,2.3,0,0,1,.59-.105c.036,0,.072,0,.109,0a2.55,2.55,0,0,1,1.212.324c2.941,1.554,1.212,7.451.561,9.672a38.306,38.306,0,0,1-3.7,8.454l-.71,1.218,18.363.808a3.916,3.916,0,0,1,3.784,3.735,3.783,3.783,0,0,1-1.123,2.834,3.629,3.629,0,0,1-2.559,1.055c-.046,0-.1,0-.145,0h-.027l-2.141-.093-9.331-.41-.075,1.7,9.333.408a3.721,3.721,0,0,1,2.666,1.3,3.855,3.855,0,0,1,.936,2.934,3.779,3.779,0,0,1-3.821,3.38c-.061,0-.122,0-.181-.005l-1.974-.082-8.9-.392-.075,1.7,8.9.39a3.723,3.723,0,0,1,2.666,1.3,3.86,3.86,0,0,1,.937,2.933,3.784,3.784,0,0,1-3.827,3.381c-.057,0-.118,0-.177,0l-1.976-.088-8.472-.372-.075,1.7,8.474.372a3.726,3.726,0,0,1,2.666,1.3,3.857,3.857,0,0,1,.935,2.933,3.782,3.782,0,0,1-3.827,3.381C-7080.2,1279.765-7080.26,1279.765-7080.317,1279.764Zm-38.4,0-.089,0a6.558,6.558,0,0,1-6.193-6.8l.907-27.293a6.446,6.446,0,0,1,2.074-4.553,6.214,6.214,0,0,1,3.954-1.672c.081,0,.17-.005.29-.005s.212,0,.292.005a6.561,6.561,0,0,1,6.192,6.8l-.907,27.293a6.441,6.441,0,0,1-2.072,4.547,6.249,6.249,0,0,1-4.261,1.681Z" transform="translate(7126.251 -1222.75)" fill="none" stroke="currentColor" stroke-width="2.5"/> </svg>',
            'liked_comment' => "<svg xmlns='http://www.w3.org/2000/svg' width='58.553' height='58.266' viewBox='0 0 58.553 58.266' class='feather active-like'> <path d='M-7080.317,1279.764l-26.729-1.173a1.657,1.657,0,0,1-1.55-1.717l1.11-33.374a4.112,4.112,0,0,1,2.361-3.6l.014-.005a13.62,13.62,0,0,1,1.978-.363h.007a9.007,9.007,0,0,0,3.249-.771c2.645-1.845,3.973-4.658,5.259-7.378l.005-.013.031-.061.059-.13.012-.023c.272-.576.61-1.289.944-1.929l0-.007c.576-1.105,2.327-4.46,4.406-5.107a2.3,2.3,0,0,1,.59-.105c.036,0,.072,0,.109,0a2.55,2.55,0,0,1,1.212.324c2.941,1.554,1.212,7.451.561,9.672a38.306,38.306,0,0,1-3.7,8.454l-.71,1.218,18.363.808a3.916,3.916,0,0,1,3.784,3.735,3.783,3.783,0,0,1-1.123,2.834,3.629,3.629,0,0,1-2.559,1.055c-.046,0-.1,0-.145,0h-.027l-2.141-.093-9.331-.41-.075,1.7,9.333.408a3.721,3.721,0,0,1,2.666,1.3,3.855,3.855,0,0,1,.936,2.934,3.779,3.779,0,0,1-3.821,3.38c-.061,0-.122,0-.181-.005l-1.974-.082-8.9-.392-.075,1.7,8.9.39a3.723,3.723,0,0,1,2.666,1.3,3.86,3.86,0,0,1,.937,2.933,3.784,3.784,0,0,1-3.827,3.381c-.057,0-.118,0-.177,0l-1.976-.088-8.472-.372-.075,1.7,8.474.372a3.726,3.726,0,0,1,2.666,1.3,3.857,3.857,0,0,1,.935,2.933,3.782,3.782,0,0,1-3.827,3.381C-7080.2,1279.765-7080.26,1279.765-7080.317,1279.764Zm-38.4,0-.089,0a6.558,6.558,0,0,1-6.193-6.8l.907-27.293a6.446,6.446,0,0,1,2.074-4.553,6.214,6.214,0,0,1,3.954-1.672c.081,0,.17-.005.29-.005s.212,0,.292.005a6.561,6.561,0,0,1,6.192,6.8l-.907,27.293a6.441,6.441,0,0,1-2.072,4.547,6.249,6.249,0,0,1-4.261,1.681Z' transform='translate(7126.251 -1222.75)' fill='none' stroke='currentColor' stroke-width='2.5'/> </svg>",
...
function messageMarkup($message_info=[])
{
...
function deleteDisappearingMessages($conversation_user_id,$user_id)
{
    global $wo, $sqlConnect, $db,$non_allowed;

    $chat = $db->where('conversation_user_id',$conversation_user_id)->where('user_id',$user_id)->getOne(T_U_CHATS);
    if (!empty($chat) && $chat->type == 'disappearing' && !empty($chat->disappearing_time)) {

        $messages = $db->where("((`from_id` = {$conversation_user_id} AND `to_id` = {$user_id} AND `deleted_two` = '0') OR (`from_id` = {$user_id} AND `to_id` = {$conversation_user_id} AND `deleted_one` = '0'))")->where('page_id',0)->where('group_id',0)->where('time',(time() - (int) $chat->disappearing_time),'<')->get(T_MESSAGES);

        foreach ($messages as $key => $value) {
            Wo_DeleteMessage($value->id);
        }
    }
}

function deleteOneTimeMessages()
{
    global $wo, $sqlConnect, $db,$non_allowed;

    $removed_messages = $db->where('seen',0,'!=')->where('type_two','one_time')->get(T_MESSAGES);
...
function isSticker($path='')
{
    global $wo, $sqlConnect, $db,$non_allowed;

    $path = substr($path, strpos($path, 'upload/'));

    return $db->where('media_file',$path)->getValue(T_STICKERS,'COUNT(*)') > 0 ? true : false;
}
